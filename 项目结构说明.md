...
- Python 3.x
- CUDA（用于GPU加速）
- Conda环境（talking_gaussian、cosyvoice等）
...
│   ├── audios/                     # 音频文件目录
│   │   ├── input.wav               # 用户录音输入
│   │   ├── tts_output.wav          # 语音合成输出
│   │   └── custom_voice/           # 自定义语音克隆参考音频
│   ├── videos/                     # 生成的视频文件
...
### 7.1 输入数据
- `TalkingGaussian/data/<ID>/<ID>.mp4`：训练视频
- `static/audios/input.wav`：用户录音
- `static/audios/custom_voice/*.wav`：自定义语音克隆参考音频

### 7.2 输出数据
- `TalkingGaussian/output/<project_name>/`：训练好的模型
- `static/videos/*.mp4`：生成的视频文件
- `static/text/input.txt`：ASR识别结果
- `static/text/output.txt`：LLM生成的回复
- `static/audios/tts_output.wav`：语音合成输出
