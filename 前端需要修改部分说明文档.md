...
3. 点击"停止录音"按钮
   └──→ 停止录音
   └──→ 自动调用 /save_audio 接口
   └──→ 保存到 static/audios/input.wav
   └──→ ⚠️ 不再自动触发对话生成

4. 填写参数表单：
   - 模型名称：选择 "TalkingGaussian" 或 "SyncTalk"
   - 模型目录地址：输入已训练的模型路径
   - 语音克隆模型：选择参考音频来源（当前录音/预设音色/自定义）
   - GPU选择：选择 GPU0/GPU1/CPU
   - 参数选择：语言类型、语速、渲染质量等
...
**文件**：`templates/chat_system.html`

**需要修改的函数**：`uploadAudio()` （约第 699-728 行）

**当前代码**（第 710-712 行）：
```javascript
.then(res => {
    if (res.status === 'success') {
        statusMessage.textContent = '录音已保存，开始生成对话...';
        triggerChat(); // ⚠️ 需要删除这行，不再自动触发对话生成
    }
```
...
- ✅ 录音停止后只上传音频，不自动触发对话
- ✅ 用户需要手动填写参数表单
- ✅ 用户点击"开始对话"按钮后才触发对话生成
- ✅ "开始对话"按钮的提交逻辑保持不变（已绑定在表单的 submit 事件上）
...
3. **实时对话页面**：
   - [ ] ⚠️ **重要**：修改 `uploadAudio()` 函数，删除自动触发对话的代码（第 712 行）
   - [ ] 实现语音克隆参考音频三种选择方式（当前录音/预设音色/自定义）
   - [ ] 添加语言类型选择器
   - [ ] 添加语速调节滑块
   - [ ] 添加渲染质量等级选择器
   - [ ] 实现自定义音频文件上传功能
   - [ ] 确保"开始对话"按钮正确绑定表单提交事件
