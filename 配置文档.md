...
打开OpenFace生成的CSV文件，检查是否包含以下AU列：
...
**方法1：直接使用 OpenFace 的输出**（如果包含所有需要的列）
```bash
# 重命名并移动到项目目录
cp <OpenFace输出目录>/<视频名>.csv au.csv
```

**方法2：提取需要的列**（如果 CSV 文件包含很多不需要的列）

创建Python脚本 `extract_au.py`：
...
# 读取 OpenFace 输出的 CSV
input_file = sys.argv[1] if len(sys.argv) > 1 else 'openface_output.csv'
output_file = sys.argv[2] if len(sys.argv) > 2 else 'au.csv'
...
if len(existing_columns) < len(au_columns):
    print(f"警告：缺少以下列：{set(au_columns) - set(existing_columns)}")

# 提取存在的列
au_df = df[existing_columns]

# 保存为 au.csv
au_df.to_csv(output_file, index=False)
print(f"已保存 {len(existing_columns)} 个AU列到 {output_file}")
```

运行脚本：
```bash
python extract_au.py <OpenFace输出目录>/<视频名>.csv au.csv
```
...
# 检查是否包含必要的 AU 列
python3 -c "import pandas as pd; df = pd.read_csv('TalkingGaussian/data/May/au.csv'); print('列数:', len(df.columns)); print('行数:', len(df)); print('列名:', df.columns.tolist()[:5])"
```
...
按照第5节说明，使用OpenFace生成 `au.csv` 文件，并上传到：
```
TalkingGaussian/data/<项目ID>/au.csv
```
...
```bash
bash run_all_metrics.sh \
  --pred_dir /app/TalkingGaussian/output/May/test \
  --gt_dir /app/TalkingGaussian/data/May/May.mp4 \
  --output_file /app/test_metrics_may.json
```
...
```bash
bash run_all_metrics.sh \
  --pred_dir /app/TalkingGaussian/output/May/train/result.mp4 \
  --gt_dir /app/TalkingGaussian/data/May/May.mp4 \
  --output_file /app/test_metrics_may.json
```
...
cd TalkingGaussian/evaluation
ls -la
# 应看到：run_all_metrics.sh, eval_frame_metrics.py 等脚本
```
