...
- **操作系统**：Ubuntu 20.04+ / CentOS 7+ / Windows 10/11（64位） / 其他Linux发行版
- **Docker**：20.10+（必须）
- **Docker Compose**：1.29+（推荐）
- **NVIDIA Docker**：支持GPU（Linux：`nvidia-docker2`，Windows：Docker Desktop with WSL 2）

### 1.3 Windows系统：启动Docker Desktop（重要！）

**在Windows系统上构建Docker镜像前，必须先启动Docker Desktop。**
...
**方法3：通过系统托盘**
- 如果Docker Desktop已安装但未运行，在系统托盘查找Docker图标并点击

#### 1.3.2 等待Docker Desktop完全启动

启动后需要等待Docker Desktop完全启动：
- **系统托盘图标**：会出现Docker图标（鲸鱼图标）
- **启动状态**：图标从"正在启动"变为"运行中"（通常为绿色）
- **启动时间**：通常需要30秒到2分钟

#### 1.3.3 验证Docker是否可用

等待Docker Desktop完全启动后，在PowerShell中执行：
...
**解决方法**：
- 确保Docker Desktop已完全启动（系统托盘图标显示为运行状态）
- 如果启动失败，检查是否已安装WSL 2（见1.3.4节）

#### 1.3.4 Windows系统要求

**必须满足以下要求**：
1. **Windows版本**：Windows 10 64位（版本2004或更高）或 Windows 11
2. **WSL 2**：必须启用WSL 2（Windows Subsystem for Linux 2）
...
# 如果未安装WSL 2，以管理员身份运行PowerShell，然后执行：
wsl --install
```

**启用虚拟化**：
- 在BIOS中启用虚拟化功能（Virtualization Technology）
- 不同主板的设置位置不同，通常在"Advanced"或"CPU Configuration"中

#### 1.3.5 如果Docker Desktop启动失败
...
### 2.4 检查嵌套子模块（`third_party/glm`，用于 CUDA 扩展编译）

`diff-gaussian-rasterization` 本身也包含一个嵌套子模块 `third_party/glm`，用于编译 CUDA 扩展。如果该目录为空，会在 Docker 构建时报错：
...
```bash
cd TalkingGaussian/submodules/diff-gaussian-rasterization

# 查看子模块状态（应能看到 third_party/glm）
git submodule status
...
如果 `third_party/glm` 为空或不存在，说明嵌套子模块没有初始化。
...
# 递归初始化所有子模块（包含嵌套的 glm）
git submodule update --init --recursive
```

如果仍然缺少 `glm`，可以在 `diff-gaussian-rasterization` 目录内单独处理：
...
**说明**：
- `.gitmodules` 顶层文件中 **不需要** 单独为 `third_party/glm` 添加配置，因为它已经在 `diff-gaussian-rasterization/.gitmodules` 中定义。
- 对老师来说，只需要按照文档在项目根目录执行一次 `git submodule update --init --recursive`，就会自动拉取包括 `glm` 在内的所有子模块。
...
**情况1：使用预构建的Docker镜像（推荐给老师）**

如果老师直接使用你构建并导出的Docker镜像：
...
# 老师只需要：
# 1. 导入镜像
gunzip -c tfg_ui_image.tar.gz | docker load
...
如果老师从GitHub克隆项目并重新构建：

⚠️ **必须先克隆子模块**
- 从GitHub克隆的项目，子模块目录通常是空的
- 必须先初始化子模块，再构建镜像
...
#### 方法1：使用官方评测脚本 eval_all.py

`eval_all.py` 是 TalkingGaussian 项目官方的评测脚本，位于 `TalkingGaussian/scripts/eval_all.py`，支持所有评测指标。

**方式1：使用包装脚本 run_eval.sh（最简单，推荐）**
...
# 方式B：使用 conda run（无需激活）
conda run -n talking_gaussian --no-capture-output \
  python /app/TalkingGaussian/scripts/eval_all.py \
    --pred-video /app/TalkingGaussian/output/May/train/result.mp4 \
    --gt-video /app/TalkingGaussian/data/May/May.mp4 \
    --all \
    --json-out /app/static/metrics.json
...
**主要参数**：
- `--pred-video`：预测视频路径（必需）
- `--gt-video`：真实视频路径（必需，用于PSNR/SSIM/FID）
- `--all`：运行所有可能的指标（PSNR/SSIM/NIQE/FID/LSE）
- `--frame`：仅运行PSNR/SSIM
- `--niqe`：仅运行NIQE
- `--fid`：仅运行FID
- `--lse`：仅运行LSE-C/LSE-D
- `--json-out`：输出JSON结果文件路径（可选）
...
#### 方法2：使用统一评测脚本 run_all_metrics.sh（备用）
