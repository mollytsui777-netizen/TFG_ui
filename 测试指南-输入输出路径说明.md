...
**输入参数**：
- **参考视频地址**（必填）
  - 示例：`TalkingGaussian/data/May/May.mp4`
  - 格式：相对路径，相对于项目根目录
  - 说明：训练用的原始视频文件
...
3. **语音克隆参考音频选择**：
   ...
   - **预设音色**：使用 `CosyVoice/asset/zero_shot_prompt.wav` 或 `CosyVoice/asset/cross_lingual_prompt.wav`
...
|---------|---------|---------|--------|----------|
| **模型训练** | `TalkingGaussian/data/<ID>/<ID>.mp4` | `TalkingGaussian/output/<project_name>/` | ❌ 无 | ✅ 覆盖同名模型 |
| **视频生成** | `static/audios/*.wav`<br>`output/<model_name>` | `static/videos/talkinggaussian_<name>_<timestamp>.mp4` | ✅ 有 | ❌ 不覆盖 |
