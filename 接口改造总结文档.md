...
| CosyVoice脚本 | `/root/TalkingGaussian/test_cosyvoice.py` | `./TalkingGaussian/test_cosyvoice.py` | `/app/TalkingGaussian/test_cosyvoice.py` |
...
subprocess.run([
    ...
    'python', '/app/TalkingGaussian/test_cosyvoice.py',
    ...
], cwd='/app')
```
...
### 3. `TalkingGaussian/test_cosyvoice.py`
**用途**：CosyVoice 语音克隆
...
curl -X POST http://localhost:5001/chat_system \
  -F "model_param=output/talking_May" \
  -F "voice_clone=voice_clone/CosyVoice-main/asset/zero_shot_prompt.wav"
```
